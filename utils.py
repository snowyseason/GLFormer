...
def get_lr_scheduler(lr_decay_type, lr, min_lr, total_iters, warmup_iters_ratio=0.05, warmup_lr_ratio=0.1,
    ...
    def yolox_warm_cos_lr(lr, min_lr, total_iters, warmup_total_iters, warmup_lr_start, no_aug_iter,
                          iters, warm_up_mode = 'linear'):
        if iters <= warmup_total_iters:
            if warm_up_mode == 'linear':
                # linear warmup
                lr = (lr - warmup_lr_start) * iters / float(warmup_total_iters) + warmup_lr_start
                # index warmup
            elif warm_up_mode == 'index':
                lr = (lr - warmup_lr_start) * pow(iters / float(warmup_total_iters), 2) + warmup_lr_start
            else:
                raise ValueError(f'Please choose the right warmup mode, {warm_up_mode} is mistaked')
    ...
    if lr_decay_type == "cos":
        ...
        func = partial(yolox_warm_cos_lr, lr, min_lr, total_iters, warmup_total_iters, warmup_lr_start,
                       no_aug_iter)
    else:
