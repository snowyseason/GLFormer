...
def configure_metadata(metadata_root):
    ...
    获取Metadata
    image_ids.txt:  图像的路径
    class_labels.txt:   图像的标签
    image_sizes.txt:    图像的大小
    localization.txt    目标的定位坐标

    :param metadata_root:   metadata的路径
    :return:    metadata
    """
    metadata = mch()
...
def get_dataset(data_roots, metadata_root, resize_size, crop_size,
                proxy_training_set, num_val_sample_per_class, dataset_name):
    ...
    datasets = {
        split: WSOLImageLabelDataset(
            data_root=os.path.join(data_roots, dataset_name),
            metadata_root=os.path.join(metadata_root, dataset_name, split),
            transform=dataset_transforms[split],
            proxy=proxy_training_set and split == 'train',
            num_sample_per_class=(
                10 if split == 'train_box'
                else num_val_sample_per_class if split == 'val'
                else 0
            )
        )
